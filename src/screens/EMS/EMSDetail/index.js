...
export default function EMSDetail({navigation, route}) {
  const NotationInputRef = useRef();

  const emsID = route?.params?.emsID;
  ...
  const getEms = async () => {
    ...
    const resp = await ApiManager.get(`ems/${emsID}`);
    if (resp.ok) {
      setEMS(resp.data);
      ...
      showMessage({
        message: 'Try Again',
        description: 'Error while fetching ems record.',
        type: 'danger',
      });
      navigation.goBack();
    }
  ...
  const getNotes = async pages => {
    const response = await ApiManager.get(`ems-notes/${emsID}`, {
      limit: 5,
      page: pages,
      sort: 'desc',
    });
  ...
  const downloadPdf = async () => {
    ...
    body = body.concat(
      `<h2 style="padding-left: 40px;">Notations: ${ems?.name}</h2>`,
    );
    body = body.concat(
      '<span style="padding: 40px">Date: ' + dayjs().format() + '</span>',
    );
    const res = await ApiManager.get(`ems-notes/${emsID}/all`);
    if (!res.ok) {
  ...
  const deleteNotation = async () => {
    ...
    setTimeout(async () => {
      ...
      const res = await ApiManager.delete(
        `ems-notes/${emsID}/${selectedNote._id}`,
      );
      if (res.ok)
          ...
          <>
            ...
            <View style={styles.innerView}>
              ...
              <DetailCard
                ...
                interfacilty={
                  ems?.interFacilityTransfer == 'dont_know'
                    ? "Don't Know"
                    : ems?.interFacilityTransfer == 'yes'
                    ? 'Yes'
                    : 'No'
                }
              />
              ...
              <LocationCard
                city={ems?.city}
                state={ems?.mailingAddressState}
                zipcode={ems.zipCode}
                latitude={ems?.zipCoords?.coordinates[1]}
                longitude={ems?.zipCoords?.coordinates[0]}
                practiceAddressState={ems?.practiceAddressState}
              />
              {myInfo?.organizationType != 'other' && myInfo?.organization && (
                <>
