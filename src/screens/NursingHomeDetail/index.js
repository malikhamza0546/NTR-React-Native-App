...
export default function NursingHomeDetail({navigation, route}) {
  const NotationInputRef = useRef();

  const nursingId = route?.params?.nursingId;
  const myInfo = useSelector(state => state.Auth.user);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [showDownloadModal, setShowDownloadModal] = useState(false);
  const [showAddModal, setShowAddModal] = useState(false);
  const [notations, setNotations] = useState([]);
  const [nursing, setNursing] = useState(null);
  ...
  const getNursingData = async () => {
    ...
    const resp = await ApiManager.get(`nursing-home/${nursingId}`);
    if (resp.ok) {
      setNursing(resp.data);
      ...
      showMessage({
        message: 'Try Again',
        description: 'Error while fetching nursing home record.',
        type: 'danger',
      });
      navigation.goBack();
    }
  ...
  const getNotes = async pages => {
    const response = await ApiManager.get(`nursing-home-notes/${nursingId}`, {
      limit: 5,
      page: pages,
      sort: 'desc',
    });
  ...
  const downloadPdf = async () => {
    ...
    body = body.concat(
      `<h2 style="padding-left: 40px;">Notations: ${nursing?.name}</h2>`,
    );
    body = body.concat(
      '<span style="padding: 40px">Date: ' + dayjs().format() + '</span>',
    );
    const res = await ApiManager.get(`nursing-home-notes/${nursingId}/all`);
    if (!res.ok) {
  ...
  const deleteNotation = async () => {
    ...
    setTimeout(async () => {
      ...
      const res = await ApiManager.delete(
        `nursing-home-notes/${nursingId}/${selectedNote._id}`,
      );
      if (res.ok)
        ...
        {nursing != null && (
          <>
            ...
            <View style={styles.innerView}>
              ...
              <DetailCard
                phoneNumber={nursing?.phoneNumber}
                certifiedBed={nursing?.certifiedBeds}
                specialPayment={nursing?.specialPayment}
                Urban={nursing?.forhpRural_Urban2020}
              />
              <CustomText
                fontWeight="bold"
              ...
              <LocationCard
                ...
                city={nursing?.city}
                state={nursing?.state}
                zipcode={nursing.zipcode}
                latitude={nursing?.zipCoords?.coordinates[1]}
                longitude={nursing?.zipCoords?.coordinates[0]}
              />
              {myInfo?.organizationType != 'other' && myInfo?.organization && (
                <>
